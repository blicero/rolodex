...
func init() {
	for _, id := range logfacility.All() {
...
// are stored.
var (
	BaseDir = filepath.Join(os.Getenv("HOME"), ".rolodex.d")
	LogPath = filepath.Join(BaseDir, "rolodex.log")
	DbPath  = filepath.Join(BaseDir, "rolodex.db")
)
